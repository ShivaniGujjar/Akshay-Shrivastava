    ...
    <footer className="fixed bottom-0 left-0 w-screen max-w-full box-border z-[9999] h-20 pointer-events-none overflow-visible bg-transparent pb-4 sm:pb-6">
      <div className="w-full relative h-full z-[15] overflow-visible">
        ...
        <motion.div 
          ...
          className="absolute right-5 bottom-4 flex items-center justify-center bg-[#144BFF] border border-white/20 px-5 sm:px-7 h-12 sm:h-14 rounded-2xl pointer-events-auto gap-3 sm:gap-5 shadow-2xl will-change-transform"
        >
          ...
          {isAtCenter && (
            <>
              ...
              <span className="text-[#FFC822]/40 text-xs select-none font-bold">
                •
              </span>
            </>
          )}
          ...
          {SOCIAL_LINKS.map((social, idx) => (
            <React.Fragment key={social.id}>
              ...
              {idx < SOCIAL_LINKS.length - 1 && (
                <span className="text-xs text-[#FFFFFF]/40 select-none pointer-events-none font-bold">
                  •
                </span>
              )}
            </React.Fragment>
